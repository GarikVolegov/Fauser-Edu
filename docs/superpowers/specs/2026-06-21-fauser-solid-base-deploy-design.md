...
*Acceptance:* `curl /api/healthz` → 200 with no Clerk keys set.
...
*Acceptance:* with no keys + `NODE_ENV=development`, authenticated routes return data
...
*Acceptance:* `vite dev` boots and shows the dashboard as a mock user with no key set.
...
*Acceptance:* a single documented command brings the full stack up locally.
...
*Acceptance:* `pnpm run lint` exits 0.
...
*Acceptance:* `pnpm run test` runs and passes; meaningful (not trivial) assertions.
...
*Acceptance:* `pnpm --filter @workspace/fauser-platform build` works with no env set.
...
*Acceptance:* Vite build emits no chunk-size warning; route chunks are split.
...
*Acceptance:* CI is green on a clean checkout.
...
*Acceptance:* committing a `dist` file is rejected by the guard.
