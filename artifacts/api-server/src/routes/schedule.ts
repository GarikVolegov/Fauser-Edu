...
router.get("/", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const entries = await db
      .select()
      .from(scheduleTable)
      .where(eq(scheduleTable.classId, classId));
...
router.post("/", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(scheduleTable.classId, classId),
          eq(scheduleTable.dayOfWeek, dayOfWeek),
          eq(scheduleTable.hour, hour),
        ),
      )
      .limit(1);
    ...
    const [entry] = await db
      ...
      .values({ classId, dayOfWeek, hour, subjectId, teacherId, room })
      .returning();
    ...
    const teacher = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, entry.teacherId))
      .limit(1);
