...
const enrichAssignment = async (a: any) => {
  ...
  const classes = await db
    .select()
    .from(classesTable)
    .where(eq(classesTable.id, a.classId))
    .limit(1);
  return {
    ...a,
  ...
  try {
    ...
    if (parsed.success) {
      if (parsed.data.classId)
        filters.push(eq(assignmentsTable.classId, parsed.data.classId));
      if (parsed.data.subjectId)
        filters.push(eq(assignmentsTable.subjectId, parsed.data.subjectId));
      if (parsed.data.upcoming)
        filters.push(
          gte(assignmentsTable.dueDate, new Date().toISOString().split("T")[0]),
        );
    }
