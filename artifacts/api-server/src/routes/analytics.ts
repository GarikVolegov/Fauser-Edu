...
import { db, usersTable, classesTable, gradesTable, attendanceTable, subjectsTable, quizzesTable } from "@workspace/db";
...
router.get("/summary", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const avgGrade = gradeValues.length > 0 ? gradeValues.reduce((a, b) => a + b, 0) / gradeValues.length : 0;

    const attendance = await db.select().from(attendanceTable);
    const present = attendance.filter(a => a.status === "presente").length;
    const attendanceRate = attendance.length > 0 ? Math.round((present / attendance.length) * 100) : 100;
    ...
    const gradesBySubject = Array.from(gradesBySubjectMap.entries()).map(([subjectId, vals]) => ({
    ...
    const attendanceByMonth = Array.from(byMonth.entries()).map(([month, { total, present }]) => ({
      month,
