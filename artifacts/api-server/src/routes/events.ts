...
router.get("/", requireAuth, async (req: any, res: any) => {
  try {
    ...
    if (parsed.success) {
      if (parsed.data.from)
        filters.push(gte(eventsTable.startDate, parsed.data.from));
      if (parsed.data.to)
        filters.push(lte(eventsTable.startDate, parsed.data.to));
    }
...
router.post("/", requireRole(["teacher", "segreteria", "admin"]), async (req: any, res: any) => {
  try {
    ...
    res
      .status(201)
      .json({ ...event, createdAt: event.createdAt.toISOString() });
  } catch (err) {
