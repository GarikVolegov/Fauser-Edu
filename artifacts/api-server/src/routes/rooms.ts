...
router.post("/", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const [room] = await db.insert(roomsTable).values({ name, capacity: capacity ?? 30, type: type ?? "aula", equipment }).returning();
...
router.get("/bookings", requireAuth, async (req: any, res: any) => {
  try {
    const filters: any[] = [];
    if (req.query.roomId) filters.push(eq(roomBookingsTable.roomId, parseInt(req.query.roomId as string)));
    if (req.query.date) filters.push(eq(roomBookingsTable.date, req.query.date as string));

    const bookings = filters.length > 0
      ? await db.select().from(roomBookingsTable).where(and(...filters))
...
router.post("/bookings", requireAuth, async (req: any, res: any) => {
  try {
    ...
    if (!roomId || !date || !startTime || !endTime || !purpose) return res.status(400).json({ error: "Missing fields" });

    const conflict = await db.select().from(roomBookingsTable)
      .where(and(eq(roomBookingsTable.roomId, roomId), eq(roomBookingsTable.date, date)))
      .then(bookings => bookings.find(b => !(endTime <= b.startTime || startTime >= b.endTime)));

    if (conflict) return res.status(409).json({ error: "Time slot already booked" });

    const [booking] = await db.insert(roomBookingsTable).values({ roomId, teacherId: user.id, date, startTime, endTime, purpose }).returning();
    const [room] = await db.select().from(roomsTable).where(eq(roomsTable.id, booking.roomId)).limit(1);
    res.status(201).json({ ...booking, roomName: room?.name ?? "Unknown", teacherName: `${user.firstName} ${user.lastName}`, createdAt: booking.createdAt.toISOString() });
  } catch (err) {
