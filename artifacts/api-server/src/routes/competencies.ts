...
router.post("/", requireRole(["teacher", "admin"]), async (req: any, res: any) => {
  try {
    ...
    const [subject] = await db
      .select()
      .from(subjectsTable)
      .where(eq(subjectsTable.id, comp.subjectId))
      .limit(1);
    res.status(201).json({
      ...comp,
...
router.get("/student", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const filters: any[] = [];
    if (studentId)
      filters.push(eq(studentCompetenciesTable.studentId, studentId));

    const items =
      filters.length > 0
        ? await db
            .select()
            .from(studentCompetenciesTable)
            .where(and(...filters))
    ...
    res.json(
      items.map((sc) => {
        ...
        return {
          ...
          subjectName: comp
            ? (subjectMap.get(comp.subjectId) ?? "Unknown")
            : "Unknown",
...
router.post("/student", requireRole(["teacher", "admin"]), async (req: any, res: any) => {
  try {
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(studentCompetenciesTable.studentId, studentId),
          eq(studentCompetenciesTable.competencyId, competencyId),
        ),
      )
      .limit(1);
    ...
    if (existing.length > 0) {
      [sc] = await db
        .update(studentCompetenciesTable)
        .set({ level, teacherId: user.id, date, notes })
        .where(eq(studentCompetenciesTable.id, existing[0].id))
        .returning();
    } else {
      [sc] = await db
        .insert(studentCompetenciesTable)
        .values({
          studentId,
          competencyId,
          level,
    ...
    const [comp] = await db
      .select()
      .from(competenciesTable)
      .where(eq(competenciesTable.id, sc.competencyId))
      .limit(1);
    const [subject] = comp
      ? await db
          .select()
    ...
    const [student] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, sc.studentId))
      .limit(1);
    const [teacher] = await db
      .select()
      .from(usersTable)
      .where(eq(usersTable.id, sc.teacherId))
      .limit(1);
