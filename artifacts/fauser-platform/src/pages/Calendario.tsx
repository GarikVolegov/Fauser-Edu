...
export default function Calendario() {
  ...
  const getEventLabel = (type: string) => {
    switch (type) {
      case "compito":
        return "Compito in classe";
      case "esame":
        return "Esame / Verifica";
      case "vacanza":
        return "Vacanza";
      case "evento_scolastico":
        return "Evento Scolastico";
      default:
        return "Altro";
    }
  };
  ...
  const selectedDateEvents =
    events?.filter((e) => date && isSameDay(new Date(e.startDate), date)) || [];

  // Function to determine if a date has events for the calendar day styling
  const modifiers = {
    hasEvents: (d: Date) =>
      events?.some((e) => isSameDay(new Date(e.startDate), d)) || false,
    hasExams: (d: Date) =>
      events?.some(
        (e) => e.type === "esame" && isSameDay(new Date(e.startDate), d),
      ) || false,
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Impegni, scadenze ed eventi scolastici.
        </p>
      </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {selectedDateEvents.map((event) => (
                  ...
                  >
                    <div
                      className={`w-1.5 rounded-full ${getEventColor(event.type)} shrink-0`}
                    />
