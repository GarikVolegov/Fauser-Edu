...
export default function Libreria() {
  ...
  const filteredMaterials = materials.filter((m: any) => {
    ...
    const matchSubject = selectedSubject ? m.subjectId === selectedSubject : true;
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Libreria Condivisa</h1>
        <p className="text-muted-foreground mt-1">Risorse e materiali didattici per l'approfondimento.</p>
      </div>
      ...
      {filteredMaterials.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-lg bg-muted/5">
          ...
          <p className="text-sm text-muted-foreground mt-1">Prova a cambiare i filtri di ricerca.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredMaterials.map((m: any) => {
            ...
            return (
              <Card key={m.id} className="flex flex-col h-full hover:shadow-md transition-shadow">
                <CardHeader className="pb-3 flex-none">
                  ...
                  <CardTitle className="text-base line-clamp-2 leading-tight" title={m.title}>
                    {m.title}
                  </CardTitle>
                </CardHeader>
                ...
                <CardFooter className="pt-0 border-t flex-none">
                  {m.fileUrl ? (
                    <Button variant="ghost" className="w-full text-primary hover:text-primary hover:bg-primary/5 mt-3" asChild>
