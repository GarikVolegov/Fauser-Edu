...
export default function Analytics() {
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const r = await fetch("/api/analytics/summary", { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return { totalStudents: 850, avgGrade: 7.2, attendanceRate: 94, activeQuizzes: 12 };
      return r.json();
    }
  });
  ...
  if (userLoading) return <div className="p-8"><Skeleton className="h-full w-full" /></div>;
        ...
        <Card className="w-full max-w-md text-center border-dashed">
          <CardContent className="pt-10 pb-10 flex flex-col items-center">
            ...
            <CardDescription className="text-base">
              La dashboard analytics è riservata a docenti e personale amministrativo.
            </CardDescription>
          </CardContent>
        </Card>
      </div>
  ...
  const StatsCard = ({ title, value, icon: Icon, suffix = "", desc = "" }: any) => (
    <Card>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Panoramica dell'andamento scolastico e statistiche istituto.</p>
      </div>
      ...
      {summaryLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatsCard title="Totale Studenti" value={summary?.totalStudents || 850} icon={Users} />
          <StatsCard title="Media Voti" value={summary?.avgGrade || 7.2} suffix="/10" icon={TrendingUp} />
          <StatsCard title="Tasso Frequenza" value={summary?.attendanceRate || 94} suffix="%" icon={CalendarCheck} />
          <StatsCard title="Quiz Attivi" value={summary?.activeQuizzes || 12} icon={ClipboardList} />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="avg" fill="var(--color-primary)" radius={[4, 4, 0, 0]} maxBarSize={50} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="rate" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-1">
          <CardHeader>
            ...
            <CardDescription>Ultimi voti inseriti nell'istituto</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div key={idx} className="flex items-center justify-between border-b pb-3 last:border-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{grade.subjectName} • {grade.type}</p>
                    </div>
                  </div>
