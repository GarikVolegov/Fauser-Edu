...
export default function Colloqui() {
  ...
  const { data: appointments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const param = user?.role === 'teacher' ? `teacherId=${user?.id}` : `studentId=${user?.id}`;
      const r = await fetch(`/api/appointments?${param}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return [];
  ...
  const { data: teacherAppointments = [] } = useQuery({
    ...
    queryFn: async () => {
      const token = await getToken();
      const r = await fetch(`/api/appointments?teacherId=${selectedTeacher}&date=${date}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return [];
      return r.json() as Promise<AppointmentType[]>;
    },
  ...
  const createAppointment = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Colloquio prenotato", description: "In attesa di conferma del docente." });
  ...
  const updateStatus = useMutation({
    mutationFn: async (data: { id: number; status: "confirmed" | "cancelled" }) => {
      const token = await getToken();
  ...
  const occupiedSlots = new Set(teacherAppointments.filter(a => a.status !== 'cancelled').map(a => a.timeSlot));

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Colloqui</h1>
        <p className="text-muted-foreground mt-1">Prenota e gestisci i colloqui con i docenti.</p>
      </div>

      <Tabs defaultValue="miei-colloqui" className="w-full">
        <TabsList className="grid w-full max-w-md grid-cols-2">
          {user?.role === 'student' && <TabsTrigger value="prenota">Prenota</TabsTrigger>}
          ...
          <TabsContent value="prenota" className="mt-6 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Seleziona un docente, una data e un orario.</CardDescription>
              </CardHeader>
                ...
                {!selectedTeacher ? (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {teachers.map((t: any) => (
                        <Card key={t.id} className="cursor-pointer hover:border-primary transition-colors" onClick={() => setSelectedTeacher(t.id)}>
                  ...
                  <div className="space-y-6 max-w-md">
                    <div className="flex items-center justify-between bg-muted/30 p-3 rounded-md">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback>{teachers.find((t: any) => t.id === selectedTeacher)?.firstName?.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <span className="font-medium">
                          {teachers.find((t: any) => t.id === selectedTeacher)?.firstName} {teachers.find((t: any) => t.id === selectedTeacher)?.lastName}
                        </span>
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => setSelectedTeacher(null)}>Cambia</Button>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Argomenti da trattare..." />
                    </div>
        ...
        <TabsContent value="miei-colloqui" className="mt-6 space-y-4">
          {appointments.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-lg bg-muted/10">
              ...
              <p className="text-sm text-muted-foreground mt-1">Non hai colloqui in programma.</p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {appointments.map((app) => (
                <Card key={app.id} className={app.status === 'cancelled' ? 'opacity-60' : ''}>
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-base">
                        {format(new Date(app.date), "dd/MM/yyyy")} alle {app.timeSlot}
                      </CardTitle>
                      {app.status === 'requested' && <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">In attesa</Badge>}
                      {app.status === 'confirmed' && <Badge variant="outline" className="bg-green-100 text-green-800 border-green-200">Confermato</Badge>}
                      {app.status === 'cancelled' && <Badge variant="outline" className="line-through text-muted-foreground">Annullato</Badge>}
                    </div>
                    <CardDescription>
                      {user?.role === 'teacher' ? `Studente: ${app.studentName}` : `Docente: ${app.teacherName}`}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {app.notes && (
