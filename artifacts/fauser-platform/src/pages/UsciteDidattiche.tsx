...
export default function UsciteDidattiche() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <Badge variant="secondary">In programmazione</Badge>;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Visualizza e partecipa alle uscite programmate.
          </p>
        </div>
      </div>
                ...
                <CardContent className="pt-6 flex-1 space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span className="truncate" title={trip.destination}>
                        {trip.destination}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span>
                        {trip.date
                          ? format(new Date(trip.date), "d MMM yyyy", {
                              locale: it,
                            })
                          : "Da definire"}
                      </span>
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span>
                        {trip.budget ? `€${trip.budget}` : "Gratuito"}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <CardFooter className="bg-muted/10 border-t pt-4">
                  <div className="w-full flex items-center justify-between">
                    ...
                    {trip.status === "published" && !trip.myStatus ? (
                      <Button
                        onClick={() => joinTrip.mutate(trip.id)}
