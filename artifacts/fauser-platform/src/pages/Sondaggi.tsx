    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Partecipa alle decisioni della classe e dell'istituto.
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="attivi">
                      ...
                      <Card className="h-full flex flex-col">
                        <CardHeader className="pb-4">
                          <div className="flex justify-between items-start gap-4 mb-2">
                            ...
                            <Badge
                              variant="outline"
                              className="bg-primary/5 text-primary shrink-0 whitespace-nowrap"
                            >
                              <Clock className="mr-1 h-3 w-3" /> Attivo
                            </Badge>
                          </div>
                          <CardDescription>
                            Creato da {poll.authorName}
                          </CardDescription>
                        </CardHeader>
                              ...
                              {poll.options.map((opt: PollOption) => {
                                ...
                                return (
                                  <div key={opt.id} className="space-y-1">
                                    ...
                                    <Progress
                                      value={percent}
                                      className={`h-2 ${isMyVote ? "[&>div]:bg-primary" : "[&>div]:bg-muted-foreground/30"}`}
                                    />
                                  </div>
                                );
                              })}
        ...
        <TabsContent value="chiusi" className="mt-6">
          <Card className="flex flex-col items-center justify-center h-48 text-center p-6 border-dashed">
            <CardDescription>
              Nessun sondaggio chiuso di recente.
            </CardDescription>
          </Card>
        </TabsContent>
      </Tabs>
