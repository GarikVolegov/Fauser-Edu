...
type TutoringPostType = {
  ...
  classId?: number; createdAt: string;
}
...
export default function Tutoraggio() {
  ...
  const [newPost, setNewPost] = useState({ subjectId: "", type: "offre", description: "" });
  ...
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const r = await fetch(`/api/tutoring`, { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return [];
  ...
  const filteredPosts = posts.filter((p: TutoringPostType) => {
    ...
    if (filterSubject !== "all" && p.subjectId.toString() !== filterSubject) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Tutoraggio tra Pari</h1>
          <p className="text-muted-foreground mt-1">Bacheca scambi: offri o cerca aiuto nello studio.</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="mr-2 h-4 w-4" /> Nuovo annuncio</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <ToggleGroup type="single" value={newPost.type} onValueChange={(v) => v && setNewPost(p => ({ ...p, type: v }))} className="justify-start">
                  <ToggleGroupItem value="offre" className="data-[state=on]:bg-green-100 data-[state=on]:text-green-800">Offro aiuto</ToggleGroupItem>
                  <ToggleGroupItem value="cerca" className="data-[state=on]:bg-amber-100 data-[state=on]:text-amber-800">Cerco aiuto</ToggleGroupItem>
                </ToggleGroup>
              </div>
              <div className="space-y-2">
                <Label>Materia</Label>
                <Select value={newPost.subjectId} onValueChange={v => setNewPost(p => ({ ...p, subjectId: v }))}>
              ...
              <Button 
                onClick={() => createPost.mutate({ subjectId: parseInt(newPost.subjectId), type: newPost.type, description: newPost.description })}
                disabled={!newPost.subjectId || newPost.description.length < 10 || createPost.isPending}
              >
                Pubblica
              </Button>
            </div>
      ...
      {filteredPosts.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-lg bg-muted/5">
          ...
          <p className="text-sm text-muted-foreground mt-1">Non ci sono annunci che corrispondono ai filtri.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPosts.map((post) => (
            <Card key={post.id} className={`flex flex-col h-full ${post.status === 'closed' ? 'opacity-50' : ''}`}>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{format(new Date(post.createdAt), "dd MMM", { locale: it })}</div>
                    </div>
                  </div>
                  {post.status === 'closed' ? (
                    <Badge variant="outline">Chiuso</Badge>
                  ) : post.type === 'offre' ? (
                    <Badge variant="outline" className="bg-green-100 text-green-800 border-green-200">Offre</Badge>
                  ) : (
                    <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">Cerca</Badge>
                  )}
                </div>
              </CardHeader>
              <CardContent className="pb-4 flex-1">
                <Badge variant="secondary" className="mb-3 bg-primary/10 text-primary hover:bg-primary/20">{post.subjectName}</Badge>
              ...
              <CardFooter className="pt-0 border-t flex-none">
                {post.authorId === user?.id && post.status !== 'closed' ? (
                  ...
                  <Button variant="ghost" className="w-full text-primary hover:text-primary hover:bg-primary/5 mt-3">
